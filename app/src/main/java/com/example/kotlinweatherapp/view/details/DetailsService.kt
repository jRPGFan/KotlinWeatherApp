...
class DetailsService(name: String = "DetailService") : IntentService(name) {
    ...
    private fun loadWeather(latitude: String, longitude: String) {
        try {
            val uri =
                URL("https://api.weather.yandex.ru/v2/informers?lat=${latitude}&lon=${longitude}")
            ...
            onMalformedURL(e.message ?: "Empty Error")
        }
    }
    ...
    private fun onMalformedURL(error: String) {
        ...
        broadcastIntent.putExtra(DETAILS_URL_MALFORMED_MESSAGE_EXTRA, error)
        LocalBroadcastManager.getInstance(this).sendBroadcast(broadcastIntent)
    }
